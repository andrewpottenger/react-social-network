...
export class UserBoxComponent extends Component<
  IUserBoxComponentProps,
  IUserBoxComponentState
> {
  /**
   * Fields
   */
  ...
  circleList = () => {
    ...
    if (circles) {
      circles.forEach((circle, circleId) => {
        let isBelong = selectedCircles
          ? selectedCircles!.indexOf(circleId!) > -1
          : false
  ...
  selectedCircleChange = (selectedCircles: ImuList<string>) => {
    ...
    if (selectedCircles.count() === userBelongCircles!.count()) {
      for (
        ...
        circleIndex < selectedCircles.count();
        circleIndex++
      ) {
  ...
  render() {
    ...
    return (
      <Paper
        key={userId}
        elevation={1}
        className={classNames('grid-cell', classes.paper)}
      >
        <div
          style={{
            display: 'flex',
        ...
        >
          <div
            onClick={() => this.props.goTo!(`/${this.props.userId}`)}
            ...
            <Button
              ...
              disabled={
                (followRequest
                  ...
                  : false) ||
                (deleteFollowingUserRequest
                  ? deleteFollowingUserRequest.status ===
                    ServerRequestStatusType.Sent
                  : false)
              }
            >
              {!isFollowed
                ? translate!('userBox.followButton')
                : belongCirclesCount! > 1
                  ? translate!('userBox.numberOfCircleButton', {
                      circlesCount: belongCirclesCount
                    })
                  : firstBelongCircle
                    ? firstBelongCircle.get('name', 'Followed')
                    : translate!('userBox.followButton')}
            </Button>
          </div>
        </div>
...
const mapDispatchToProps = (
  ...
  return {
    ...
    deleteFollowingUser: (followingId: string) =>
      dispatch(circleActions.dbDeleteFollowingUser(followingId)),
    setSelectedCircles: (userId: string, circleList: string[]) =>
      dispatch(circleActions.setSelectedCircles(userId, circleList)),
    removeSelectedCircles: (userId: string, circleList: string[]) =>
      dispatch(circleActions.removeSelectedCircles(userId)),
    openSelectCircles: (userId: string) =>
      dispatch(circleActions.openSelectCircleBox(userId)),
    closeSelectCircles: (userId: string) =>
      dispatch(circleActions.closeSelectCircleBox(userId)),
...
const mapStateToProps = (
  ...
  const addToCircleRequestId = StringAPI.createServerRequestId(
    ServerRequestType.CircleAddToCircle,
    ownProps.userId
  )
  const addToCircleRequest = state.getIn([
    'server',
    'request',
    addToCircleRequestId
  ])
  const deleteFollowingUserRequestId = StringAPI.createServerRequestId(
    ServerRequestType.CircleDeleteFollowingUser,
    ownProps.userId
  )
  const deleteFollowingUserRequest = state.getIn([
    'server',
    'request',
    deleteFollowingUserRequestId
  ])
  ...
  const isSelecteCirclesOpen = state.getIn(
    ['circle', 'openSelecteCircles', ownProps.userId],
    []
  )
  ...
  return {
    ...
    firstBelongCircle: userBelongCircles
      ? circles.get(userBelongCircles.get(0), Map({}))
      : Map({}),
...
export default connect(mapStateToProps, mapDispatchToProps)(withStyles(
  styles as any
)(UserBoxComponent as any) as any)
